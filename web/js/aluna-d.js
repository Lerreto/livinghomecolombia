...
(function () {
  ...
  document.querySelectorAll('a[href^="#"]').forEach(function (a) {
    a.addEventListener("click", function (ev) {
      ...
      ev.preventDefault();
      window.scrollTo({
        top: t.getBoundingClientRect().top + window.scrollY - 4,
        behavior: "smooth",
      });
    });
  });
  ...
  form.querySelectorAll("[data-choice-group]").forEach(function (g) {
    g.addEventListener("click", function (ev) {
      ...
      if (!c) return;
      g.querySelectorAll(".choice").forEach(function (x) {
  ...
  function waLang() {
    return (window.alunaI18n && window.alunaI18n.current()) || "es";
  }
  function waURL(text) {
  ...
  function render() {
    ...
    var txt =
      window.alunaI18n && window.alunaI18n.t
        ? window.alunaI18n.t(key, window.alunaI18n.current())
        : null;
    nextBtn.querySelector(".t").textContent =
      txt || (last ? "Send me the guide" : "Continue");
  }
  ...
  nextBtn.addEventListener("click", function () {
    ...
    if (current < steps.length - 1) {
      ...
      sendLeadToWhatsApp();
      if (window.sendLeadToEspo) sendLeadToEspo(); // -> envía el lead a EspoCRM
